...
public class PostMessageApi {
	...
	@PostMapping("/editPmg") //수정하기 버튼 누를때
	public ResponseEntity<ResponseDTO<PostMessage>> editPmgSend(@AuthenticationPrincipal GeneralUser gerneralUser,
			@RequestBody PostMessage pmg) throws Exception{
		Member m = memberService.findAccount(gerneralUser.getId());
		pmg.setMemberId(gerneralUser.getId());
		pmg.setTrainerId(trainerService.findByUsername(pmg.getRecieveName()));
		if(m != null) {
...
//	@GetMapping("/getAll") //목록..다보여주는 매서드 이나 사용 안 할꺼같음
//	public ResponseEntity<ResponseDTO<PostMessage>> getAll(@AuthenticationPrincipal GeneralUser gerneralUser) throws Exception{
//		Member m = memberService.findAccount(gerneralUser.getId());
//		if(m != null) {
//			List<PostMessage> viewList = postMsgService.getAllMessages();
//					
//			ResponseDTO<PostMessage> responseDTO = ResponseDTO.<PostMessage>builder().data(viewList).build();
//			return ResponseEntity.ok().body(responseDTO);
//		}else {
